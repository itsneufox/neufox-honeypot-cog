...
- Once configured, the cog watches all messages. If a member speaks in the honeypot channel they are banned, their triggering message is deleted, and Discord automatically removes up to a day of their recent messages via `delete_message_days=1`.
...
3. **Multiple honeypot channels?** — currently only one channel per guild can be configured; rerun `[p]honeypot` to change it.
